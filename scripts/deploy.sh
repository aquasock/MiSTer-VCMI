...
# Usage: scripts/deploy.sh [code|data|tools|all]   (default: code)
...
code() {
	"${SSH[@]}" "mkdir -p $DEVICE_DIR && rm -rf $DEVICE_DIR/bin $DEVICE_DIR/libs $DEVICE_DIR/data/config $DEVICE_DIR/data/Mods"
